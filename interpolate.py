...
parser.add_argument(
    "--input_video", type=str, required=True, help="Path/WebURL to input video"
)
parser.add_argument(
    "--youtube-dl", type=str, help="Path to youtube_dl", default=".local/bin/youtube-dl"
)
parser.add_argument(
    "--factor",
    type=int,
...
if args.output_ext != ".avi":
    print(
        "Currently supporting only writing to avi. Try using ffmpeg for conversion to mp4 etc."
    )
...
def loadModel(model, checkpoint):
    ...
    saved_state_dict = {
        k.partition("module.")[-1]: v for k, v in saved_state_dict.items()
    }
...
def make_image(img):
    q_im = img.data.mul(255.0).clamp(0, 255).round()
...
def files_to_videoTensor(path, downscale=1.0):
    ...
    print(len(files))
    images = [
        torch.Tensor(np.asarray(Image.open(os.path.join(input_video, f)))).type(
            torch.uint8
        )
        for f in files
    ]
...
idxs = (
    torch.Tensor(range(len(videoTensor)))
    .type(torch.long)
    .view(1, -1)
    .unfold(1, size=nbr_frame, step=1)
    .squeeze(0)
)
